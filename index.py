...
def read_panda_file_question_2():
    ...
    for row, counter in zip( covid_simple_titles['P_name'], covid_simple_titles['covid_weekly_count']):
        if provider == row:
            ...
            if provider_list != [] and max(count_list) != max(count_list): ###
                print(max(count_list))
            provider = row
            provider_list.append(provider)

    print(provider_count_list)
    #print(provider_list)

    print(provider_list[:10])
    ...
    """
    for provider, count in zip(provider_list, covid_count):
        print(provider, count)
    """

    print(f"The largest amount of weeks where the covid count was zero is {max(count_list)} weeks")
